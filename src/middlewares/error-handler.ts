 ...
 */
function getHttpStatusCode({ error, response }: { error: Error | Record<string, any>; response: { statusCode: number } }) : number {
    /**
     * Check if the error object specifies an HTTP
     * status code which we can use.
     */
    const statusCodeFromError = (error as { status: number; statusCode: number }).status || (error as { statusCode: number }).statusCode;
